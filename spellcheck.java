...
public class spellcheck {
	...
	public static void checkWord(String inputWord, int index) {
		...
		if (index < inputWord.length()
				&& inputWord.charAt(index - 1) == inputWord.charAt(index)) {
			int count = 0;
			...
			checkWord(
					inputWord.substring(0, index)
							+ inputWord.substring(index + count), index);

			if (index - 1 + count < inputWord.length()) {
				// System.out.println(inputWord.substring(0, index - 1) +
				// inputWord.substring(index));
				checkWord(
						inputWord.substring(0, index - 1)
								+ inputWord.substring(index), index);
			}
		...
		if (vowelLocation >= 0) {
			for (int i = 0; i < vowels.length(); i++) {
				...
				if (count == word.length() - 1)
					...
					word = word.substring(0, count) + vowels.charAt(i)
							+ word.substring(count + 1, word.length());
